...
- ```magicBox```是基于```springboot+shiro+jwt```的真正```restful URL```资源无状态认证权限管理系统的后端,前端[usthe](https://gitee.com/tomsun28/usthe)  
...
这个nginx作为反向代理服务器，解决了跨域,真实IP头(现服务器端支持跨域可不用nginx)。另一个nginx作为angular应用服务器，tomcat作为magicBox的服务器。  
...
- clone 项目到本地 git clone https://gitee.com/yourName/magicBox.git
- 用idea导入
- 更改开发环境mysql数据库和redis地址(前提安装数据库并导入usthe.sql创建数据库usthe)
- 运行magicBoxApplication
- magicBox就可以提供api了 http://localhost:8080
- 推荐使用postman进行api调试
...
- 在项目目录下 docker build -t magicBox:1.0 . 
- docker images看是否生成镜像成功
- 运行 docker run -d -p 8080:8080 --name haiGirl magicBox:1.0
...
- 参照搭建完成后,magicBox对应的jenkins下运行shell:
````
#!/bin/bash
...
gitee(main rep):  
 ...
 [magicBox](https://gitee.com/tomsun28/magicBox)  
 [usthe](https://gitee.com/tomsun28/usthe) 
 
github: 

[magicBox](https://github.com/tomsun28/magicBox)  
